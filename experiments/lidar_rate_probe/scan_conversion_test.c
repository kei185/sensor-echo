...
static void test_two_points(void)
{
        ...
        const uint8_t packet[] = {
                0xaau, 0x55u, 0u, 2u, 1u, 0u, 1u, 0x2du, 0u, 0u,
                0u,    0x90u, 0x01u, 0u, 0x20u, 0x03u,
        };
        ...
        assert(tx_scan_frame_write(
                       meter.packet, meter.packet_length, tx, sizeof(tx), 0x12345678u) ==
               sizeof(tx));
        const uint8_t expected_payload[] = {100u, 0u, 0u, 0u, 200u, 0u, 0x80u, 0x16u};
        assert(memcmp(tx + TX_FRAME_HEADER_SIZE, expected_payload, sizeof(expected_payload)) ==
               0);
        ...
        assert(tx[6] == 0x12u && tx[7] == 0x34u && tx[8] == 0x56u &&
               tx[9] == 0x78u);

        // Invalid or incomplete packets must not make a PC frame.
        assert(tx_scan_frame_write(packet, sizeof(packet) - 1u, tx, sizeof(tx), 0u) == 0u);
        assert(tx_scan_frame_write(packet, sizeof(packet), tx, sizeof(tx) - 1u, 0u) == 0u);
        uint8_t bad_packet[sizeof(packet)];
        memcpy(bad_packet, packet, sizeof(packet));
        bad_packet[0] = 0u;
        assert(tx_scan_frame_write(bad_packet, sizeof(bad_packet), tx, sizeof(tx), 0u) == 0u);
        memcpy(bad_packet, packet, sizeof(packet));
...
static void test_largest_packet(void)
{
        uint8_t packet[SYS_PACKET_SCAN_FIXED_SIZE + 255u * SYS_PACKET_POINT_DATA_SIZE] = {
                0xaau, 0x55u, 0u, 255u, 1u, 0u, 1u, 0u, 0u, 0u,
        };
